...
def check_state():
	return 1 if state_has_changed("(((ADC(0).read()) >= 700) and ((ADC(0).read()) <= 900))", (((ADC(0).read()) >= 700) and ((ADC(0).read()) <= 900))) else 0

def check_order():
	return 1 if state_has_changed("(((ADC(0).read()) >= 500) and ((ADC(0).read()) <= 600))", (((ADC(0).read()) >= 500) and ((ADC(0).read()) <= 600))) else 0
...
def main():
    ...
    global queueO
    global mqttms
    Room = 'Restaurant1/1/req'
    delay = 1

    mqtt.set_callback(onmessage)
    mqtt.connect()
    mqtt.subscribe(b'Restaurant1/1')
    while True:
      if True:
          mqtt.check_msg()
          if state == 0:
          ...
          if state == 3:
                  ...
                  if (check_order() == 1 and check_alive() != 1):
                      mqtt.publish(Room,str('reqO'),retain=False)
                      state = 4
                      time.sleep_ms(delay)
                  elif (check_order() != 1 and check_alive() == 1):
